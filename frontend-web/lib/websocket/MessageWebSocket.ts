...
// NOTE: MessageWebSocket is deprecated - use NotificationWebSocket instead (shared endpoint)
// Keeping for backwards compatibility, but it delegates to NotificationWebSocket
import { notificationWS } from './NotificationWebSocket';

export const messageWS = {
  connect: (token: string) => {
    // Delegate to NotificationWebSocket (shared endpoint)
    notificationWS.connect(token);
  },
  disconnect: () => notificationWS.disconnect(),
  on: (event: WSMessageType, handler: (...args: any[]) => void) => {
    notificationWS.on(event, handler);
    return () => notificationWS.off(event, handler);
  },
  sendMessage: (type: WSMessageType, data: any) => {
    // Not needed for messages - uses REST API
  },
  isConnected: () => notificationWS.isConnected(),
};

// Auto-start ping interval (keep connection alive)
if (typeof window !== 'undefined') {
  setInterval(() => {
    if (messageWS.isConnected()) {
      messageWS.sendMessage('ping', {});
    }
